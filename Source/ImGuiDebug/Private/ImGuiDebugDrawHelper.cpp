...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawArc(
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawSphere(
    const UWorld* InWorld,
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawFlatCapsule(
    const UWorld* InWorld,
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawFrustum(
    const UWorld* World,
    const FMatrix& Matrix,
    const float Angle,
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawQuad(const UWorld* World, const FVector& Position, const FQuat& Rotation, const FVector2D& Extents, const FColor& Color, bool bPersistent, float LifeTime, uint8 DepthPriority, const float Thickness)
{
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawSolidQuad(const UWorld* World, const FVector& Position, const FQuat& Rotation, const FVector2D& Extents, const FColor& Color, bool bPersistent, float LifeTime, uint8 DepthPriority)
{
    const FVector U = Rotation.GetAxisZ() * Extents.X;
    const FVector V = Rotation.GetAxisY() * Extents.Y;
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawHitResult(
    const UWorld* World,
    const FHitResult& HitResult,
    const FImGuiDebugDrawLineTraceParams& Settings)
{
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawHitResults(
    const UWorld* World,
    ...
    const FImGuiDebugDrawLineTraceParams& Settings)
{
    ...
    {
    	// Avoid drawing the same thing multiple times
    	auto mutableSettings = Settings;
    	if (AlreadyDrawnActors.Contains(HitResult.GetActor())) mutableSettings.DrawHitPrimitiveActorsName = false;
    	if (AlreadyDrawnPrimitives.Contains(HitResult.GetComponent())) mutableSettings.DrawHitPrimitives = false;

    	DrawHitResult(World, HitResult, mutableSettings);

    	if (Settings.DrawHitPrimitives)
    	{
    		AlreadyDrawnPrimitives.Add(HitResult.GetComponent());
    		if (Settings.DrawHitPrimitiveActorsName)
    		{
    			AlreadyDrawnActors.Add(HitResult.GetActor());
    		}
    	}
    }
}
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawLineTrace(
    const UWorld* World,
    const FVector& Start,
    const FVector& End,
    const bool HasHits,
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawSweep(
    const UWorld* World,
    ...
    const bool HasHits,
    TArray<FHitResult>& HitResults,
    const FImGuiDebugDrawSweepParams& Settings
)
{
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawOverlap(
    const UWorld* World,
    const FCollisionShape& Shape,
    const FVector& Location,
    const FQuat& Rotation,
    TArray<FOverlapResult>& OverlapResults,
    const FImGuiDebugDrawOverlapParams& Settings
)
{
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawPrimitiveComponent(
...
//--------------------------------------------------------------------------------------------------------------------------
void FImGuiDebugDrawHelper::DrawShape(
    const UWorld* World,
