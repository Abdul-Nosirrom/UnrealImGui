...
//----------------------------------------------------------------------------------------------------------------------
const UObject* FImGuiWindowHelper::GetFirstAssetByClass(const TSubclassOf<UObject> AssetClass)
{
...
//--------------------------------------------------------------------------------------------------------------------------
FString FImGuiWindowHelper::GetActorName(const AActor* Actor)
{
	if (Actor == nullptr)
	{
		return FString("none");
...
#if WITH_EDITOR
	const FImGuiDebugSettings& Settings = FImGuiDebug::Settings;
...
//-----------------------------------------------------------------------------------------
bool FImGuiWindowHelper::ComputeBoundingBoxScreenPosition(const APlayerController* PlayerController, const FVector& Origin, const FVector& Extent, FVector2D& Min, FVector2D& Max)
{
    FVector Corners[8];
